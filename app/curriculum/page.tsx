...
const cardStyle = (status: string) => {
    ...
    return "bg-zinc-50/50 border border-zinc-100 opacity-60"
  }
        ...
        <div className="flex justify-between items-center w-full px-8 py-4 max-w-screen-2xl mx-auto">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex gap-6 items-center font-headline tracking-tight text-sm font-semibold">
              ...
              <a className="text-red-800 border-b-2 border-red-800 pb-1" href="#">Curriculum</a>
  ...
  <span className="material-symbols-outlined text-lg">logout</span> Sign Out
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {submissions.map((m) => (
                  <div key={m.id} className={`p-4 rounded-xl flex flex-col items-center justify-between min-h-[140px] relative transition-all ${cardStyle(m.status)}`}>
                    <div className={`absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${badgeStyle(m.status)}`}>
                      <span className="material-symbols-outlined text-[10px]" style={m.status === "approved" ? { fontVariationSettings: "'FILL' 1" } : {}}>
                        {badgeIcon(m.status)}
                      </span>
                      {m.status}
                    </div>
                    <div className="mt-4 text-center">
                      ...
                      <p className="text-[9px] text-zinc-400 font-medium uppercase tracking-tighter leading-tight">{m.milestone?.course?.name}</p>
                    </div>
