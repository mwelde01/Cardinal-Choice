...
export default function SharingPage() {
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center w-full px-8 py-4 max-w-screen-2xl mx-auto">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex gap-6 items-center font-headline tracking-tight text-sm font-semibold">
              ...
              <a className="text-red-800 border-b-2 border-red-800 pb-1" href="#">Sharing</a>
            </div>
          </div>
      ...
      {/* Main Content */}
      <main className="lg:ml-64 pt-28 pb-12 px-6 md:px-12 max-w-5xl">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-8 ghost-shadow border border-zinc-100">
              ...
              <div className="flex items-center bg-surface-container-low rounded-xl p-2 pl-5 gap-3">
                <span className="material-symbols-outlined text-zinc-400 text-sm">link</span>
            ...
            <div className="bg-white rounded-2xl p-8 ghost-shadow border border-zinc-100">
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <Toggle enabled={portfolioPublic} onToggle={() => setPortfolioPublic(!portfolioPublic)} />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Toggle enabled={allowDownload} onToggle={() => setAllowDownload(!allowDownload)} />
                </div>
                <div className="h-px bg-zinc-100"></div>
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <Toggle enabled={showContact} onToggle={() => setShowContact(!showContact)} />
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={link.email} className={`flex items-center justify-between p-4 rounded-xl border ${link.active ? 'bg-zinc-50 border-zinc-100' : 'bg-zinc-50/50 border-zinc-100 opacity-60'}`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs ${link.active ? 'bg-primary/10 text-primary' : 'bg-zinc-200 text-zinc-400'}`}>
                        {link.name.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-zinc-800">{link.name}</p>
                        <p className="text-[10px] text-zinc-400">{link.email}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className={`text-[10px] font-bold uppercase tracking-wide ${link.active ? 'text-green-600' : 'text-zinc-400'}`}>
                        {link.expires}
                      </span>
                      <button className="p-1.5 hover:bg-zinc-200 rounded-lg transition-colors">
                        <span className="material-symbols-outlined text-zinc-400 text-sm">more_horiz</span>
                      </button>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            ...
            <div className={`rounded-2xl p-6 border ${portfolioPublic ? 'bg-green-50 border-green-100' : 'bg-zinc-50 border-zinc-200'}`}>
              <div className="flex items-center gap-3 mb-3">
                <span className={`material-symbols-outlined ${portfolioPublic ? 'text-green-600' : 'text-zinc-400'}`} style={{ fontVariationSettings: "'FILL' 1" }}>
            ...
            <div className="bg-white rounded-2xl p-6 ghost-shadow border border-zinc-100">
              <h3 className="text-sm font-bold font-headline uppercase tracking-widest text-zinc-400 mb-5">Recent Access</h3>
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-surface-container-low flex items-center justify-center flex-shrink-0">
                      <span className="material-symbols-outlined text-zinc-400 text-sm">{log.icon}</span>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-zinc-700">{log.who}</p>
                      <p className="text-[10px] text-zinc-400">{log.when}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-surface-container-low rounded-2xl p-6">
              <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400 mb-4">Tips</h3>
      ...
      <nav className="md:hidden fixed bottom-0 w-full bg-white border-t border-zinc-100 flex justify-around py-3 z-50">
        <a className="flex flex-col items-center gap-1 text-zinc-400" href="/dashboard">
